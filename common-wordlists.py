...
STRIPPED_DICT_COMM = "Stripped wordlist:"
...
STRIP_WORD_COMM = "Strip all except words."
ANALYZE_ORIGIN_COMM = "Resulting file will not be in the same order as in the first wordlist."
...
def read_wordlist_4_analyze(DIC_NAME, len_dict,analyze_wd):        
    print ()
    print(LOADING_DICT_COMM_ANALYZE, DIC_NAME,"("+str(len_dict), WORDS_COMM+")",end='\r')
    result_wordlist_tmp = []
    count =0    
    print ()
    try:
        with open(DIC_NAME, 'r') as fin:
            for line in fin:
                ...
                print(SET_WORD_ORDER_COMM, word_to_sort_len_percent,"% -",count,WORDS_COMM, end='\r')            
...
def searching_for_common_words(dict1_set, dict2_set):
    ...
    for counter, i in enumerate(dict1_set, start=1):
        print(str(counter)+"|"+str(line_counter)+"|"+str(found), FOUND_COMM+"|"+str(number_of_lines_per_second_max),
              WORDS_COMM+"/s|"+REMAINING_COMM, str(datetime.timedelta(seconds=sec_to_end))+" s"+"|"+str(i)+"    " * 10, end='\r', flush=True)
        sec_now = int(datetime.datetime.now().second)
        try:
            sec_to_end = round((line_counter-counter) /
                               number_of_lines_per_second_max)
        except:
            sec_to_end = 0
...
def check_strip_par(dict):
    assemblage = set()
    for i in dict:
        assemblage.add(
            re.sub("[!\"#$%&'()*+,-./:;<=>?@[\]^_`{|}~0123456789]", '', i))
    return assemblage
...
parser = argparse.ArgumentParser(description="""The program is used to find identical words in two large wordlists.
...
if strip:
    ...
    print(STRIPPED_DICT_COMM, DICT2_NAME,
          "("+str(len(dict2_set)), WORDS_COMM+")\n")
